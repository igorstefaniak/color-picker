...
class SettingsDialog(QDialog):
    def __init__(self, parent=None, settings=None):
        ...
        app_title = QLabel("KDE Color Picker")
...
class ColorPicker(QWidget):
    def __init__(self, pick_on_start: bool = True):
        ...
        self.settings = QSettings("color-picker-kde", "ColorPicker")
        default_history = []
...
def update_desktop_shortcut(shortcut_str: str):
    ...
    paths = [
        os.path.join(home, ".local/share/applications/color-picker-kde.desktop"),
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "color-picker-kde.desktop")
    ]
    for path in paths:
        if os.path.exists(path):
            try:
                ...
                for i, line in enumerate(lines):
                    if line.strip().startswith("X-KDE-Shortcuts="):
                        lines[i] = f"X-KDE-Shortcuts={shortcut_str}\n"
                        updated = True
                        break
                ...
                if not updated:
                    ...
                    for idx, line in enumerate(lines):
                        if line.strip() == "[Desktop Entry]":
                            lines.insert(idx + 1, f"X-KDE-Shortcuts={shortcut_str}\n")
                            inserted = True
                            break
                    if not inserted:
                        lines.append(f"X-KDE-Shortcuts={shortcut_str}\n")
                
                with open(path, "w", encoding="utf-8") as f:
                    f.writelines(lines)
            except Exception as e:
                print(f"Error saving shortcut to {path}: {e}")

    # Refresh KDE system shortcut registry
    import subprocess
...
def main():
    ...
    app.setDesktopFileName("color-picker-kde")
    app.setQuitOnLastWindowClosed(False)

    import getpass
    socket_name = f"color-picker_socket_{getpass.getuser()}"
    ...
    if not server.listen(socket_name):
        print("Local server error:", server.errorString())

    pick_on_start = "--no-pick" not in sys.argv
    window = ColorPicker(pick_on_start=pick_on_start)
